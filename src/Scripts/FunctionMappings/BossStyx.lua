...
function mod.HadesKillPresentation(unit, args)
	...
	StopAnimation({ DestinationId = game.CurrentRun.Hero.ObjectId, Name = "HadesReverseDarknessVignetteHold" })
	StopAnimation({ DestinationId = game.CurrentRun.Hero.ObjectId, Name = "HadesReverseDarknessGroundFog" })
	SetAlpha({ Ids = GetIds({ Name = "Terrain_Lighting_01" }), Fraction = 1.0, Duration = 0.5 })
...
function mod.ModsNikkelMHadesBiomesHandleHadesCastDeath(projectileData, triggerArgs)
	...
	if victim ~= nil and victim.ObjectId == game.CurrentRun.Hero.ObjectId and not victim.ModsNikkelMHadesBiomesBoilingBloodShieldActive and game.TableLength(game.CurrentRun.Hero.InvulnerableFlags or {}) == 0 then
