...
function mod.HarpyKillPresentation(unit, args)
	...
	if encounterData.HoldKillPresentationForUnitDeaths ~= nil then
		local unitIds = GetIdsByType({ Names = encounterData.HoldKillPresentationForUnitDeaths }) or {}
		for k, id in pairs(unitIds) do
			if game.ActiveEnemies[id] ~= nil and not game.ActiveEnemies[id].IsDead then
				allBossesDead = false
				break
			end
		end
	...
	game.RecordEncounterClearStats()
	game.LockCameraMotion("HarpyKillPresentation")
	...
	if unit.BossKillVoiceLines ~= nil then
		game.thread(game.PlayVoiceLines, unit.BossKillVoiceLines, false, unit)
	end
	...
	game.AddTimerBlock(game.CurrentRun, "HarpyKillPresentation")
	...
	game.ToggleCombatControl(game.CombatControlsDefaults, false, "BossKill")
	...
	if deathPanSettings.FlashRed then
		Flash({ Id = victimId, Speed = 0.01, MinFraction = 1.0, MaxFraction = 1.0, Color = game.Color.Red, Duration = 0.5, ExpireAfterCycle = true })
	end
	...
	if killerId == game.CurrentRun.Hero.ObjectId then
		game.SetPlayerFade({ Flag = "KillPresentation" })
	else
	...
	game.thread(game.DisplayInfoBanner, nil,
		{
			...
			FadeColor = game.Color.LocationTextGold,
	...
	game.wait(deathPanSettings.EndPanTime or 5.5)
	...
	if killerId == game.CurrentRun.Hero.ObjectId then
		game.ClearPlayerFade("KillPresentation")
	else
	...
	game.UnlockCameraMotion("HarpyKillPresentation")
...
function mod.HarpyBuildRage(enemy, weaponAIData, currentRun)
	...
	Flash({
		Id = game.ScreenAnchors.BossRageFill,
		Speed = 3.0,
		MinFraction = 0,
		MaxFraction = 0.8,
