modutil.mod.Path.Context.Wrap.Static("DamageEnemy", function(victim, triggerArgs)
	modutil.mod.Path.Wrap("ProcessHealthBuffer", function(base, enemy, damageEventArgs)
		if enemy.ModsNikkelMHadesBiomesIsThanatosCursed and not (damageEventArgs.AttackerTable ~= nil and damageEventArgs.AttackerTable.ObjectId == game.CurrentRun.Hero.ObjectId) then
			return false
		end
