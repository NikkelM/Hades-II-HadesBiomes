modutil.mod.Path.Context.Wrap("SetupHarvestPoints", function(currentRoom, harvestPointArgs)
	...
	modutil.mod.Path.Wrap("GetInactiveIdsByType", function(base, args)
		...
		if game.CurrentRun.ModsNikkelMHadesBiomesIsModdedRun and game.CurrentRun.CurrentRoom and game.CurrentRun.CurrentRoom.RoomSetName ~= "Chaos" then
			if game.Contains({ "HarvestPoint", "ShovelPoint", "PickaxePoint", "ExorcismPoint" }, args.Name) then
				local innerCurrentRoom = game.CurrentRun.CurrentRoom
				local eligibleIds = {}
				local ineligibleSpawnPointIds = {}
				-- Get a list of predetermined spawn points set in the room's encounter that we can't use
				if innerCurrentRoom.Encounter and innerCurrentRoom.Encounter.SpawnWaves then
					for _, wave in ipairs(innerCurrentRoom.Encounter.SpawnWaves) do
						if wave.Spawns then
				...
				local usedIds = {}
				for _, usedId in ipairs(innerCurrentRoom.ExorcismPointChoices or {}) do
					usedIds[usedId] = true
				end
				for _, usedId in ipairs(innerCurrentRoom.ShovelPointChoices or {}) do
					usedIds[usedId] = true
				end
				for _, usedId in ipairs(innerCurrentRoom.HarvestPointChoicesIds or {}) do
					usedIds[usedId] = true
				end
				for _, usedId in ipairs(innerCurrentRoom.PickaxePointChoices or {}) do
					usedIds[usedId] = true
				end
