local hadesCodexTextFile = rom.path.combine(mod.hadesGameFolder, "Content\\Game\\Text\\fr\\CodexText.fr.sjson")
...
for oldName, newName in pairs(mod.HadesCodexTextNameMappings) do
	mod.UpdateField(hadesCodexTextTable.Texts, oldName, newName, { "Id" }, "CodexText.fr.sjson")
end

-- The Hades II CodexText file is not large enough to handle the hook
local screenTextFile = rom.path.combine(rom.paths.Content(), 'Game\\Text\\fr\\HelpText.fr.sjson')
