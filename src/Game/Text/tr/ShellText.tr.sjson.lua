local shellTextFile = rom.path.combine(rom.paths.Content(), "Game\\Text\\tr\\ShellText.tr.sjson")
			...
			for key, value in pairs(modifiedEntry) do
				...
				elseif value.Mode == "Replace" then
					...
					mod.DebugPrint("Unknown modification mode '" ..
						tostring(value.Mode) .. "' for entry Id '" .. entry.Id .. "' in ScreenText.tr.sjson", 4)
				end
			end
		end
	end
