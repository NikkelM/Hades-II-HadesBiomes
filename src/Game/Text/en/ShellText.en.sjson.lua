local shellTextFile = rom.path.combine(rom.paths.Content(), "Game\\Text\\en\\ShellText.en.sjson")
...
sjson.hook(shellTextFile, function(data)
	...
	for _, entry in ipairs(data.Texts) do
		...
		if modifiedEntry then
			mod.DebugPrint("Modifying ShellText entry Id '" .. entry.Id .. "'", 4)
			for key, value in pairs(modifiedEntry) do
				...
				elseif value.Mode == "Replace" then
					...
					mod.DebugPrint("Unknown modification mode '" ..
						tostring(value.Mode) .. "' for entry Id '" .. entry.Id .. "' in ScreenText.en.sjson", 4)
				end
			end
		end
	end
