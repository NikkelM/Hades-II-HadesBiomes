local shellTextFile = rom.path.combine(rom.paths.Content(), "Game\\Text\\es\\ShellText.es.sjson")
			...
			for key, value in pairs(modifiedEntry) do
				...
				elseif value.Mode == "Replace" then
					...
					mod.DebugPrint("Unknown modification mode '" ..
						tostring(value.Mode) .. "' for entry Id '" .. entry.Id .. "' in ScreenText.es.sjson", 4)
				end
			end
		end
	end
